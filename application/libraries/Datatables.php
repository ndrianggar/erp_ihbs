...
class Datatables {
    ...
    private function get_filtering() {
        $AvColumns = array_unique(explode(",", $this->ci->input->post('sColumns'))); #PENAMBAHAN ARRAY UNIQUE ADALAH UNTUK MENGANTISIPASI JIKA ADA ARRAY VALUE YANG SAMA (Av Modify)
        $sColumns = array_values($AvColumns);

        $like = array('local' => array(), 'global' => array());
        # loop through each column
        if(strpos($this->ci->input->post('sSearch'),"|") === FALSE){ #Av Modify
          for ($i = 0; $i < count($sColumns); $i++) { # Av Modify SEMEBLUMNYA => $this->ci->input->post('iColumns')
          ...
          if ($this->ci->input->post('sSearch') && $this->ci->input->post('sSearch') != "") {
            $like['global'] = array_merge($like['global'], $this->_applyColumnFilter($sColumns, $this->ci->input->post('sSearch')));
          }
        }
    ...
    private function _createColumnsFilterStatement($like) {
        $str = "";
        # local filter
        $totalColumns = count(array_unique(explode(",", $this->ci->input->post('sColumns')))); #Av Modify
        // echo "<pre>";
        // print_r($like['global']);
        // echo "</pre>";
        if (!empty($like['local'])) {
        ...
        if (!empty($like['global'])) {
          if(count($like['global']) != $totalColumns){
            $str = $str . ($str != "" ? ' AND ' : '') . "(" . implode(" AND ", $like['global']) . ")";
          }else{
        ...
        if ($str != "") {
            $this->ci->db->where("(" . $str . ")"); #SEBELUMNYA OR_WHERE();
        }
        return TRUE;
    }
    ...
    private function _applyColumnFilter($columnKey, $searchParams) {
        ...
        $arr = array();
        if(is_array($columnKey)){ #Av Modify
          $i=0;
          foreach ($search_terms as $key => $ele) {
            $arr[] = $columnKey[$i++]. " LIKE '%" . $ele . "%'";
          }
        }else{
          foreach ($search_terms as $key => $ele) {
